...
load_env() {
  ...
  [[ "${SPRINTCTL_DB}" == "$ROOT/.sprintctl/sprintctl.db" ]] || fail "SPRINTCTL_DB must point inside this repo"
  [[ "${KCTL_DB}" == "$ROOT/.kctl/kctl.db" ]] || fail "KCTL_DB must point inside this repo"
}
